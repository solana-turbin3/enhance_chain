...
fn test_add_new_transaction() {
    let mut full_per_db = FullPerDB { full_per_db: Vec::new() };
...
fn test_add_multiple_transactions_different_apps() {
    ...
    println!("Added signatures for different apps: {:?} -> {:?}, {:?} -> {:?}", app_program_id1, signature1, app_program_id2, signature2);
    ...
    println!("Retrieved signatures: {:?} -> {:?}, {:?} -> {:?}", app_program_id1, signatures1, app_program_id2, signatures2);
...
fn test_get_signature_for_nonexistent_app() {
    ...
    let signatures = full_per_db.get_signature_for_add(app_program_id);
    println!("Retrieved signatures for nonexistent app: {:?} -> {:?}", app_program_id, signatures);
