...
use crate::{
    chain_entrypoint::{
        transaction_context::TransactionContext, tx_entrypoint::TransactionsOnThread,
    },
    line_up_queue::line_up_queue::{AccountInvolvedInTransaction, LineUpQueue},
    processor::transaction::{TransactionMetadata, TransactionType},
...
fn test_full_flow() {
    ...
    chain_trnasaction
        .account_previlage_normalization_and_previlage_checker(transaction_account_meta.clone());
    chain_trnasaction
        .account_previlage_normalization_and_previlage_checker(transaction_account_meta_2.clone());
    chain_trnasaction
        .account_previlage_normalization_and_previlage_checker(transaction_account_meta_3.clone());
    ...
    chain_trnasaction.push_new_transaction_to_the_main_queue(
        &mut lineup_queue,
    ...
    chain_trnasaction.push_new_transaction_to_the_main_queue(
        &mut lineup_queue,
        transaction_account_meta_2,
        transaction_metadata_2,
        &mut app_user_base,
        program_id,
        "user1".to_string(),
    );
    chain_trnasaction.push_new_transaction_to_the_main_queue(
        &mut lineup_queue,
        transaction_account_meta_3,
        transaction_metadata_3,
        &mut app_user_base,
        program_id,
        "user1".to_string(),
    );
    ...
    assert_eq!(lineup_queue.lineup_queue.len(), 3);

    assert_eq!(chain_trnasaction.chain_transaction.len(), 3);

    chain_trnasaction.take_out_individual_transaction_and_apply_RWlocks(
        &mut lineup_queue,
        &mut thread_aware_locks,
        &mut transaction_on_thread,
        &mut thread_load_counter,
    );

    assert_eq!(transaction_on_thread.trnasaction_on_thread.len(), 3);

    assert_eq!(lineup_queue.reschedable_txs.len(), 0);
