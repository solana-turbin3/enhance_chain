use crate::{
    line_up_queue::line_up_queue::{AccountInvolvedInTransaction, LineUpQueue},
    processor::{
        engine::PayTubeChannel,
        setup::{system_account, TestValidatorContext},
        transaction::{TransactionMetadata, TransactionType},
    },
    scheduler::read_write_locks::{ThreadAwareLocks, ThreadLoadCounter},
...
use super::transaction_context::{InstructionContext, TransactionContext};
...
impl ChainTransaction {
    pub fn convert_account_meta_to_acc_inv_txs(
        &mut self,
        accounts: Vec<AccountsMeta>,
    ...
    pub fn account_previlage_normalization_and_previlage_checker(
        &mut self,
        accounts_meta_for_the_per_transaction: Vec<AccountsMeta>,
    ) {
        // TODO://
        // Currently only support transafer transaction
        ...
        per_transaction_context.handle_transaction_context(
            per_instruction_context,
            accounts_meta_for_the_per_transaction,
            native_writable_privilages,
        );
    }
    ...
    pub fn push_new_transaction_to_the_main_queue(
        &mut self,
        lineup_queue: &mut LineUpQueue,
        account: Vec<AccountsMeta>,
        transaction_metadata: TransactionMetadata,
        app_user_base: &mut AppUserBase,
        program_id: Pubkey,
        user_name: String,
    ) {
        ...
        let new_transaction = self.create_new_transaction(
            hashed_id,
            account_involved_in_transaction,
            1,
            match transaction_metadata.txs_type {
                TransactionType::Transfer => TransactionMetadata {
                    ...
                    args: vec![transaction_metadata.args[0]],
                },
            },
            app_user_base,
            program_id,
    ...
    pub fn take_out_individual_transaction_and_apply_RWlocks(
        &mut self,
        lineup_queue: &mut LineUpQueue,
        thread_aware_locks: &mut ThreadAwareLocks,
        transaction_on_thread: &mut TransactionsOnThread,
        thread_load_counter: &mut ThreadLoadCounter,
    ) {
        let transactions: Vec<_> = lineup_queue.lineup_queue.iter().cloned().collect();

        for transaction in transactions {
            let is_writeable_accounts_clone =
                transaction.txs_accounts.is_writeable_accounts.clone();
            let non_writeable_accounts_clone =
                transaction.txs_accounts.non_writeable_accounts.clone();
            ...
            if let Some(scheduled_thread) = thread_aware_locks.try_lock_account(
                is_writeable_accounts_clone.clone(),
                non_writeable_accounts_clone.clone(),
                thread_load_counter,
            ) {
                transaction_on_thread
                    .trnasaction_on_thread
                    .insert(transaction.id, scheduled_thread);
            } else {
                lineup_queue.add_transaction_to_non_rescheduable_container(
                    transaction.id,
                    transaction.txs_accounts,
    ...
    pub fn get_all_transaction_on_a_thread(
        &mut self,
        tsx_on_thread: TransactionsOnThread,
        thread_id: usize,
    ) -> Vec<&MakeTransaction> {
        let ids: Vec<u64> =
            TransactionsOnThread::get_all_tx_ids_for_thread(&tsx_on_thread, thread_id);
    ...
    pub fn process_all_transaction_from_threads(
        &mut self,
        tsx_on_thread: TransactionsOnThread,
        thread_id: usize,
    ) {
        let transaction_on_thread_1 =
            self.get_all_transaction_on_a_thread(tsx_on_thread, thread_id);
        println!("side_tx_res{:?}", transaction_on_thread_1);
        let transaction_metadata =
            get_all_transaction_metadata_from_transaction(transaction_on_thread_1.clone());
...
pub fn get_all_transaction_metadata_from_transaction(
    transaction: Vec<&MakeTransaction>,
        ...
        match transaction.transaction_metadata.txs_type {
            TransactionType::Transfer => {
                transaction_metadata = TransactionMetadata {
                    ...
                    args: vec![transaction.transaction_metadata.args[0]],
                };
            }
        }
...
pub fn prepare_key_for_trnasactions(
    transaction: Vec<&MakeTransaction>,
    payer: Keypair,
) -> Vec<Keypair> {
...
pub fn prepare_account_for_the_transaction(
    transaction: Vec<&MakeTransaction>,
) -> Vec<(Pubkey, solana_sdk::account::AccountSharedData)> {
