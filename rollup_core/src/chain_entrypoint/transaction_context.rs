...
impl InstructionContext {
    ...
    pub fn create_native_instruction_account_for_transaction(
        &mut self,
    ...
    pub fn create_main_instruction_account_for_transaction(
        &mut self,
        index_in_transaction: usize,
        is_signer: bool,
        is_writeable: bool,
        context_stack_height: usize,
    ) {
        let instruction_account = InstructionAccount {
            index_in_transaction,
...
impl TransactionContext {
    ...
    pub fn handle_transaction_context(
        &mut self,
        instruction_context: &mut InstructionContext,
        transaction_accounts: Vec<AccountsMeta>,
        native_writeable_privilages: Vec<bool>,
    ) {
        self.fill_accounts(transaction_accounts.clone());
        self.create_native_and_main_ins_account(
            transaction_accounts.clone(),
            instruction_context,
            native_writeable_privilages,
        );
        for account in transaction_accounts {
            // now checks the account privilages
            self.check_for_accounts_permission_previlage_mismatch(
    ...
    pub fn create_native_and_main_ins_account(
        &mut self,
        account_meta: Vec<AccountsMeta>,
        instruction_context: &mut InstructionContext,
        native_writeable_privilages: Vec<bool>,
    ) {
        ...
        for (index, accounts_meta) in account_meta.clone().iter().enumerate() {
            ...
            if duplicate_account.contains_key(&accounts_meta.key) {
                let index_of_the_instruction_account =
                    duplicate_account.get(&accounts_meta.key).unwrap();
                let mut instruction_account = *instruction_context
                    .instruction_accounts
                    .get(*index_of_the_instruction_account)
                    .unwrap();
                ...
                instruction_context.create_native_instruction_account_for_transaction(
                    index_in_transaction,
                    true,
                    native_writeable_privilages[index],
                );
                instruction_context.create_main_instruction_account_for_transaction(
                    index_in_transaction,
                    account_meta[index].is_signer,
                    account_meta[index].is_writeable,
                    stack_height,
                );
                duplicate_account.insert(
                    accounts_meta.key,
                    instruction_context.instruction_accounts.len() - 1,
                );
            }
        }
    }
    ...
    pub fn find_index_of_the_account(&mut self, account: &Pubkey) -> usize {
        let index = self
            .account_keys
            .iter()
            .position(|key| &key.key == account)
            .unwrap();
        index
    }

    // match account privilage checks from the native and user_provided instruction_account
    pub fn check_for_accounts_permission_previlage_mismatch(
        &mut self,
        instruction_context: InstructionContext,
        accounts: AccountsMeta,
    ) {
        for instruction_account in instruction_context.instruction_accounts.iter() {
            let native_instruction = self
                .get_native_ins_account(instruction_context.clone(), accounts.key)
                .unwrap();
    ...
    pub fn get_native_ins_account(
        &mut self,
        instruction_context: InstructionContext,
        account: Pubkey,
    ) -> Option<InstructionAccount> {
        let index_in_trasaction = self.find_index_of_the_account(&account);
        for (_index, native_instruction) in
            instruction_context.instruction_accounts.iter().enumerate()
        {
            if native_instruction.index_in_callee == None
                && native_instruction.index_in_transaction == index_in_trasaction
            {
...
pub mod test {
    ...
    use crate::chain_entrypoint::{
        transaction_context::TransactionContext, tx_entrypoint::AccountsMeta,
    };
    ...
    fn test_transaction_context_flow() {
        ...
        transaction_context.handle_transaction_context(
            &mut instruction_context,
            transaction_account_meta,
            native_writeable_privilage,
        );
    }
