...
impl LineUpQueue {
    pub fn add_to_main_tx_queue(
        &mut self,
        id: u64,
        txs_accounts: AccountInvolvedInTransaction,
        priority: u64,
    ) {
        self.main_queue.push(TransactionsInQueue {
            id,
            txs_accounts,
            priority,
        });
    ...
    pub fn handle_transaction(
        ...
        handle_main_queue: bool,
        trnansaction: TransactionsInQueue,
    ) {
    ...
    pub fn add_transaction_to_non_rescheduable_container(
        ...
        if self.rescheduable_budget < TOTAL_RESCHEDUABLE_BUDGET {
            self.reschedable_txs.push(TransactionsInQueue {
                id,
                txs_accounts,
                priority,
            });
